"""Build the 'real graph' for tester-c (Health & Science) using the KGBuilder pipeline.
...
Identical methodology to tester-b-real-graph (build_real_graph.py). Raw connector
labels are normalised to the system's 16 canonical relation names via REL_MAP;
canonical names are identity-mapped so build_stats' "unmapped" list only reflects
truly-unknown labels. The corpus deliberately uses the exact phrasings that the
IS-09/IS-10 fixes target ("Spring follows winter.", "Bonjour translates to hello.",
"The heart is part of the circulatory system.", "Insulin is a type of hormone.")
so this build doubles as a regression gate for those fixes.
"""
import json
import sys
...
CORPUS = OUT / "datasets" / "health_science_source.txt"
DB_PATH = OUT / "tester_c_real_graph.db"
...
REL_MAP = {
    ...
    "synonym": "synonym",
    "antonym": "antonym",
    "temporal_coincident": "temporal_coincident",
    "spatial_near": "spatial_near",
    "linguistic_maps": "linguistic_maps",
}
