"""
Parallel incremental KG builder.

Splits an article range across N worker processes, each building
an independent chunk DB via subprocess. After all workers finish,
sequentially merges all chunk DBs into the main DB.

Usage:
    python scripts/build_cnn_parallel.py --start 3000 --end 6000 --workers 3
"""
import sys, os, time, shutil, subprocess, argparse, logging, json
...
TEMP_DIR = BASE / "model_training" / "dataset_cnn" / "parallel_tmp"
ARTICLES_PER_WORKER = 1000


def setup_logging():
    log_file = BASE / "model_training" / "dataset_cnn" / "build_parallel.log"
...
def main():
    parser = argparse.ArgumentParser(description="Parallel CNN KG builder")
    ...
    print(f"PARALLEL KG BUILD — articles {args.start} to {args.end}, {args.workers} workers")
    print(f"  Main DB: {OUT_DB}")
    print(f"  Registry: {ENTITY_REG_PATH.parent}")

    # Build ranges
    chunk_size = ARTICLES_PER_WORKER
    ranges = []
    for s in range(args.start, args.end, chunk_size):
        e = min(s + chunk_size, args.end)
        if e > s:
            ranges.append((s, e))
    logger.info("Split into %d worker chunks", len(ranges))
    ...
    # Launch workers (at most `workers` at a time)
    T_GLOBAL = time.time()
    procs = []  # (process, start, end, wdir, out_db, out_er, out_rr, log_handle)
    pending = list(ranges)
    active = []
    results = []
    ...
    def launch_worker(s, e):
        ...
        cmd = [
            sys.executable, str(BASE / "scripts" / "build_cnn_incremental.py"),
    ...
    # Poll: wait for any worker to finish, collect result, launch next
    last_progress = {}
    while active:
        still_active = []
        for item in active:
        ...
        active = still_active
        if active:
            # Show live progress from active workers
            lines = []
            for p, s, e, wdir, out_db, out_er, out_rr, log_fh in active:
                try:
                    ...
                    # Find the last "Articles:" progress line
                    for line in reversed(text.splitlines()):
                        if "Articles:" in line:
                            prog = line.strip()
                            break
                    else:
                        prog = "(no progress yet)"
                except Exception:
                    prog = "(reading...)"
    ...
    # ── Merge all successful worker results into main DB ──
    print(f"\n{'='*60}")
    ...
    for s, e, wdir, chunk_db, chunk_er, chunk_rr in results:
        ...
        try:
            ...
            # Absorb chunk entity registry into main entity registry
            ...
            # Add nodes (skip existing labels)
            ...
            # Add edges (translate chunk node labels → main store node IDs)
            ...
            # Absorb chunk relation clusters into main RR
    ...
    # BGE for new nodes missing embeddings
    ...
    main_store.set_metadata("dataset_name", "cnn_dailymail")
    main_store.set_metadata("last_article", str(args.end - 1))
    ...
    # Cleanup — force close DB connections before removing temp files
    ...
    print(f"  Workers:  {len(results)}/{len(procs)} succeeded")
    print(f"  DB:       {main_store.get_node_count()} nodes, {main_store.get_edge_count()} edges ({mb:.1f} MB)")
