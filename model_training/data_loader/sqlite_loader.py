...
class SqliteLoader(DataLoader):
    ...
    def load(self, path: str, **kwargs) -> LoadedData:
        ...
        logger.info(f"SqliteLoader: {len(triples)} triples, {len(sentences)} sentences in {elapsed:.2f}s")
        return LoadedData(sentences=sentences, triples=triples,
                         metadata={"source": str(path), "format": "sqlite",
                                   "num_entries": len(triples) + len(sentences),
                                   "load_time_seconds": round(elapsed, 2), **metadata_info})
