...
# ─── Large Graph: 1000 nodes, 10000 edges ─────────────────────

DOMAINS = [
    "Programming", "AI_ML", "Database", "Networking", "Math",
    "Physics", "Chemistry", "Biology", "Security", "Systems",
    "Graphics", "UI_UX", "Compiler", "OS", "Distributed",
    "Testing", "DevOps", "Embedded", "Robotics", "NLP",
]
...
def build_scale_graph():
    ...
    return g, all_ids, domain_nodes, len(g.edges)


def run_scale_test():
    test_start = time.time()

    print("Building 1000-node, 10000-edge graph...")
    graph, all_ids, domain_nodes, edge_count = build_scale_graph()
    print(f"  Built: {len(graph.nodes)} nodes, {edge_count} edges")
    ...
    print(f"  Buffer size: {engine.replay_buffer_size} -> PASS")
    ...
    output_path = os.path.join(OUTPUT_DIR, "scale_result.json")
